...
def create_builder_pie_chart(_df):
    ...
    _df.to_csv(FOLDER + f"pie_total_relay.csv", index=None)
    _df = df[["relay", "block_hash"]].groupby(["relay"]).count().reset_index()
    _df.to_csv(FOLDER + f"pie_mevboost_relay.csv", index=None)
    ...
    _df.to_csv(FOLDER + f"pie_mevboost.csv", index=None)
    df = df[["builder_pubkey", "slot"]].groupby(["builder_pubkey"]).count().reset_index()
    df.to_csv(FOLDER + f"pie_total.csv", index=None)
...
def create_sankey(df):
    ...
    pd.DataFrame(zip(source,target,value), 
                 columns=["source", "target", "value"]).to_csv(FOLDER + "sankey_source_target_value.csv", index=None)
    pd.DataFrame(colors_builder + relay_colors, columns=["colors"]).to_csv(FOLDER + "sankey_colors.csv", index=None)
    pd.DataFrame(colors, columns=["link_colors"]).to_csv(FOLDER + "sankey_link_colors.csv", index=None)    
    pd.DataFrame(builder_labels, columns=["labels"]).to_csv(FOLDER + "builder_labels.csv", index=None)
    pd.DataFrame(all_relays, columns=["labels"]).to_csv(FOLDER + "relay_labels.csv", index=None)
...
def create_avg_val_gas_chart(_df):
    ...
    df.to_csv(FOLDER + "avg_proposer_val_gas.csv", index=None)
...
def create_potential_mev_chart():
    df = pd.read_csv(ENRICHED_DATA_FOLER + "mevboost_er.csv").sort_values("block_number")
    ...
    df.to_csv(FOLDER + "potentialmev.csv")
...
create_avg_val_gas_chart(df[~df["block_hash"].isna()])
